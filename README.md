...
A Python script to programmatically toggle **+48V Phantom Power** on **Arturia MiniFuse** audio interface.
...
2.  **Arturia MiniFuse Control Center** (Must be installed)
...
By default, the script looks in the standard installation directory:
...
**The Solution:** Instead of attempting to inject raw USB packets via filter drivers, this script loads the official Thesycon/Arturia API DLL (`arturiaminifuseusbaudioapi_x64.dll`). It invokes the exported function `TUSBAUDIO_AudioControlRequestSet` to send specific Vendor Requests to the hardware.

## Troubleshooting

*   **`Error: DLL not found`**: Verify that Arturia MiniFuse Control Center is installed. If installed to a non-default location, edit the `DLL_PATH` variable in the script.
